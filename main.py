...
# Bekor qilish uchun umumiy matn
...
TEXTS = {
    "uz": {
        ...
        "send_photo_only": "⚠️ Iltimos, rasmni albatta 'Photo' (siqilgan rasm) sifatida yuboring, fayl (document) sifatida emas.",
    ...
    "ru": {
        ...
        "send_photo_only": "⚠️ Пожалуйста, отправьте фото как 'Photo', а не как файл (document).",
    ...
    "en": {
        ...
        "send_photo_only": "⚠️ Please send the image as a 'Photo', not as a document/file.",
...
async def admin_cancel_add(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(TEXTS[lang]["cancelled"], reply_markup=main_menu_keyboard(lang, user_id))
...
async def admin_get_title_invalid(message: Message):
    lang = user_languages.get(message.from_user.id, "uz")
    await message.answer(TEXTS[lang]["send_text_only"])
...
async def admin_evidence_wrong_type(message: Message):
    lang = user_languages.get(message.from_user.id, "uz")
    await message.answer(TEXTS[lang]["send_photo_only"])
